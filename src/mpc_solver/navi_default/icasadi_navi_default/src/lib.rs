...
extern "C" {
    fn init_interface_navi_default();
    ...
    fn preconditioning_www_navi_default (
        arg: *const *const c_double,
    ) -> c_int;
    fn init_penalty_function_navi_default(
        arg: *const *const c_double,
        ipnlt: *mut *mut c_double,
    ) -> c_int;
    fn get_w_cost_navi_default() -> c_double;
} // END of extern C
...
pub fn get_w_cost() -> f64 {
    unsafe {
        get_w_cost_navi_default() as f64
    }
}
...
pub fn precondition(
    ...
    let arguments = &[u.as_ptr(), static_params.as_ptr()];
    unsafe {
         preconditioning_www_navi_default(arguments.as_ptr()) as i32
    }
}
...
pub fn initial_penalty(
    ...
    // preconditioning_init_penalty_function_navi_default
    let arguments = &[u.as_ptr(), static_params.as_ptr()];
    let ip = &mut [rho_init as *mut c_double];

    unsafe {
         init_penalty_function_navi_default(
            arguments.as_ptr(),
            ip.as_mut_ptr(),
        ) as i32
    }
