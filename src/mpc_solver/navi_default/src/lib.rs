...
const MAX_DURATION_MICROS: u64 = 1000000;
...
pub const NAVI_DEFAULT_NUM_PARAMETERS: usize = 2658;
...
fn unscale_result(solver_status: &mut Result<AlmOptimizerStatus, SolverError>) {
    if let Ok(sss) = solver_status {
        let w_cost : f64 = icasadi_navi_default::get_w_cost();
...
pub fn solve(
    ...
    icasadi_navi_default::init_navi_default();
    ...
    if DO_PRECONDITIONING {
        ...
        icasadi_navi_default::precondition(u, p);

        // Compute initial penalty
        icasadi_navi_default::initial_penalty(u, p, & mut rho_init);
    }
